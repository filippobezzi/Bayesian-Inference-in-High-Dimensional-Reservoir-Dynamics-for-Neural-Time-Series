...
def plot_probabilistic_forecast(
    ...
    y_lower,
    y_upper,
    plot_title="Probabilistic forecast",
    num_steps=None,
):
    ...
    ax.fill_between(
        time_steps, y_lower, y_upper, color="blue", alpha=0.2, label=r"95% CI"
    )
...
def plot_functional_connectivity(
    y_true_multi, y_pred_multi, title="Functional Connectivity Comparison"
):
    ...
    fig, ax = plt.subplots(
        1, 3, figsize=(12, 6), gridspec_kw={"width_ratios": [1, 1, 0.05]}
    )
    fig.suptitle(title)
    ...
    # Heatmap prediction
    sns.heatmap(
        corr_pred, vmin=-1, vmax=1, cmap="vlag", ax=ax[1], cbar=True, cbar_ax=ax[2]
    )
...
class ComplexRadar:
    def __init__(self, fig, variables, ranges, n_ring_levels=5):
        ...
        axes = [
            fig.add_axes([0.1, 0.1, 0.9, 0.9], polar=True)
...
def plot_parallel_coordinates_ci(
    metrics_data, bounds=None, colors=None, figsize=(8, 5)
):
    """
    ...
    data_mean = {
        m: np.array([np.mean(metrics_data[met][m]) for met in metrics]) for m in models
    }
    data_upper = {
        m: np.array([np.max(metrics_data[met][m]) for met in metrics]) for m in models
    }
    data_lower = {
        m: np.array([np.min(metrics_data[met][m]) for met in metrics]) for m in models
    }
    ...
    # Add padding to axes
    ymins = [
        y - 0.05 * (ymaxs[i] - y) if ymaxs[i] != y else y - 0.05
        for i, y in enumerate(ymins)
    ]
    ymaxs = [
        y + 0.05 * (y - ymins[i]) if ymins[i] != y else y + 0.05
        for i, y in enumerate(ymaxs)
    ]
    ...
    for j, model in enumerate(models):
        ...
        scaled_lower = np.array(
            [scale(data_lower[model][i], i) for i in range(n_metrics)]
        )
        scaled_upper = np.array(
            [scale(data_upper[model][i], i) for i in range(n_metrics)]
        )
        scaled_mean = np.array(
            [scale(data_mean[model][i], i) for i in range(n_metrics)]
        )
        ...
        patch_fill = patches.PathPatch(
            Path(verts_fill, codes_fill), facecolor=c, alpha=0.2, lw=0
        )
...
def plot_calibrated_test(Y_test_array, y_pred, adj_quantiles):
    region_idx = 0
