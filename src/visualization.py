...
def plot_radar_chart(metrics_data, title="MConfigurations Tradeoff", colors=None):
    """
    Genera un radar chart (spider chart) per confrontare diversi modelli su più metriche.
    
    Args:
        metrics_data (dict): Dizionario annidato nel formato {metrica: {modello: [valori]}}.
        title (str): Titolo del grafico.
        colors (list, optional): Lista di colori da usare per i modelli. Se None, usa una palette di default.
    """
    # Imposta una palette di colori di default se non fornita
    if colors is None:
        colors = ['blue', 'orange', 'green', 'red', 'purple', 'cyan', 'magenta']
        
    metrics = list(metrics_data.keys())
    # Estraiamo i modelli dalla prima metrica
    models = list(metrics_data[metrics[0]].keys())
    
    # Calcoliamo la media per ogni metrica e per ogni modello
    data_matrix = {model: [np.mean(metrics_data[metric][model]) for metric in metrics] for model in models}
    
    # Calcolo dei range per ogni asse del radar chart (con il 10% di padding)
    ranges = []
    ...
    # NOTA: Questa funzione assume che la classe ComplexRadar sia definita nel tuo ambiente
    radar = ComplexRadar(fig, metrics, ranges)
    
    # Plottiamo ogni modello
    for i, model in enumerate(models):
        # Usiamo il modulo per evitare errori se ci sono più modelli che colori nella lista
        color = colors[i % len(colors)] 
        radar.plot(data_matrix[model], label=model, color=color, linewidth=2)
        radar.fill(data_matrix[model], facecolor=color, alpha=0.15)
        
    # Formattazione
    radar.ax.set_title(title, pad=25, fontsize=14, fontweight='bold')
    radar.use_legend(loc='lower center', bbox_to_anchor=(0.5, -0.2), ncol=len(models), frameon=False)
